...
1) حيثيات الموضوع: **لا تكتبها** — تُبنى آليًا خارج هذا الطلب من نص محضر القسم حرفيًا (ثبت
   أن أي محاولة لإعادة صياغتها أو محاكاة سابقة قد تُبدّل اسم الشخص أو تفاصيله بالخطأ، وهو
   غير مقبول في مستند رسمي). تجاهل هذا الحقل تمامًا وركّز جهدك على الحقول التالية.
...
def draft_topic(engine: Engine, dept_topic, department_name: str, dept_session_number,
    ...
    # الحيثيات تُبنى دائمًا حرفيًا من محضر القسم، بغض النظر عن وجود سابقة أو قوتها: ثبت
    # عمليًا (بملف حقيقي) أن النموذج المحلي الصغير قد يخترع اسمًا مختلفًا تمامًا حتى مع
    # وجود سابقة قوية ونص كامل الطول (فشل تفوّق فيه على فحوصات الطول ودرجة التشابه معًا) —
    # أي محاولة "محاكاة أسلوب سابقة + استبدال حقائق" غير موثوقة بما يكفي لمستند رسمي.
    # النموذج يبقى مسؤولاً فقط عن التوصية/المستند/المرفقات/الإجراء (أقل حساسية للتفاصيل).
