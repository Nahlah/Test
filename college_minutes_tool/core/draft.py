...
1) حيثيات الموضوع: إن وُجدت في "مواضيع سابقة مشابهة" سابقة قريبة فعلًا لنفس نوع الموضوع، اتّبع
   بنيتها وصياغتها وترتيب فقراتها بدقة (لا تخترع أسلوبًا جديدًا)، لكن استبدل كل التفاصيل
   المتغيّرة (الاسم، الرتبة، القسم، الأرقام، التواريخ، العناوين) بما ورد **حرفيًا** في محضر
   القسم الحالي فقط — لا تنقل أي اسم أو تاريخ من السابقة نفسها إلى الموضوع الجديد. تبدأ عادة
   بعبارة مثل "بتوصية من مجلس قسم {القسم} في جلسته {رقم الجلسة بالحروف} المنعقدة بتاريخ
   {التاريخ}، ...". إن لم توجد سابقة قريبة كافية، أعد صياغة حيثيات محضر القسم بأسلوب الكلية
   (مثلاً "ناقش المجلس" بدل "استعرض المجلس") مع نفس التفاصيل تمامًا دون حذف أو إضافة.
